...
export function EditSpreadModal() {
  const editSpreadModalOpen = useUIStore((s) => s.editSpreadModalOpen);
  const setEditSpreadModalOpen = useUIStore((s) => s.setEditSpreadModalOpen);
  const updateTx = useUpdateTransaction();
  const { data: strategies = [] } = useStrategies();

  const strategyId = editSpreadModalOpen?.strategyId ?? null;
  const legs = editSpreadModalOpen?.legs ?? [];
  ...
  useEffect(() => {
    if (shortLeg && longLeg) {
      setSymbol((shortLeg.instrumentSymbol ?? '').toUpperCase());
      setExpiration((shortLeg.option?.expiration ?? '').slice(0, 10));
      setShortStrike(String(shortLeg.option?.strike ?? ''));
      setLongStrike(String(longLeg.option?.strike ?? ''));
      setQuantity(String(shortLeg.quantity ?? ''));
      setShortPremium(String(shortLeg.price ?? ''));
      setLongPremium(String(longLeg.price ?? ''));
      setDate((shortLeg.timestamp ?? '').slice(0, 10));
      const cp = (shortLeg.option?.callPut ?? 'call').toLowerCase();
      setSpreadType(cp === 'put' ? 'put' : 'call');
      setError(null);
    }
  }, [shortLeg, longLeg]);
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      e.preventDefault();
      if (!strategyId || !shortLeg || !longLeg || !strategy) return;
      ...
      const customData = (shortLeg.customData ?? {}) as Record<string, unknown>;
      const spreadId = customData.spreadId ?? crypto.randomUUID();
      const customDataWithSpreadId = { ...customData, spreadId };
      ...
      try {
        await updateTx.mutateAsync({
          id: String(shortLeg.id),
          strategyId,
          side: 'sell',
          cashDelta: shortPremiumTotal,
          currency: strategy.baseCurrency ?? shortLeg.currency ?? 'USD',
          timestamp,
          ...
          customData: customDataWithSpreadId,
          quantity: qty,
          price: shortPr,
        });

        await updateTx.mutateAsync({
          id: String(longLeg.id),
          strategyId,
          side: 'buy',
          cashDelta: -longPremiumTotal,
          currency: strategy.baseCurrency ?? longLeg.currency ?? 'USD',
          timestamp,
          instrumentSymbol: sym,
          option: {
            expiration: expIso,
        ...
        setError(err instanceof Error ? err.message : 'Failed to update spread');
        return;
      }

      setEditSpreadModalOpen(null);
    },
    [
      strategyId,
      strategy,
  ...
  if (!editSpreadModalOpen || !strategyId || legs.length !== 2 || !shortLeg || !longLeg) return null;
  ...
  return (
    <Modal title="Edit Credit Vertical Spread" onClose={handleClose} size="default">
      <form onSubmit={handleSubmit}>
        <p className="text-[13px] mb-4" style={{ color: 'var(--color-text-muted)' }}>
          Edit spread in <strong>{strategy?.name}</strong>
        </p>
